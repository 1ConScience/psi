...
class Texte(pygame.sprite.Sprite):
    def __init__(self,txt,x,y):
        super().__init__()
        self.surf = my_font.render(txt, False, (255,0,0))
...
folder_dir = glob.escape(home) + "/Pictures/11"
 
liste_images = []

# iterate over files in
# that directory
for img in glob.iglob(f'{folder_dir}/*'):
...
for index, image in enumerate(liste_images):
    tableau = Tableau(image,WIDTH+index*300, HEIGHT-100)
...
PT1 = Platform((20000, 20),(255,255,255),(0, HEIGHT))
all_sprites.add(PT1)
platforms.add(PT1)

never_txt = Texte("Quel endroit magnifique...",0, HEIGHT-100)
